    ...
    run() {
        ...
        stream
            ...
            .on('data', async (transactionPromises) => {
                try {
                    let transactionGroups = await Promise.all(transactionPromises);
                    let transactions = transactionGroups.reduce((allTransactions, transactions) => (allTransactions.concat(transactions)), []);
